...
process_animation_rotation <- function(elements,
                                       anim_types,
                                       df_anims,
                                       index,
                                       max_time_s,
  ...
  # Set initial `rotation` for any rows where initial is `TRUE`
  df_anims <-
    df_anims %>%
  ...
  # Upgrade keyword-based `anchor` values in `df_anims`
  anchor_vals <-
    df_anims$anchor %>%
  ...
  keyframes <-
    ...
    paste_right(x_right = paste0(" ", anchor_vals)) %>%
